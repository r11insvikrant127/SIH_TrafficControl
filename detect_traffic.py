...
class TrafficAnalyzer:
    def __init__(self, video_source="traffic_sample.mp4", config_file="traffic_config.json"):
        # Load configuration
        ...
        self.signal_states = [False] * 16  # Change from 4 to 16 signals (4 intersections × 4 directions)
        ...
        # Vehicle tracking - improved with better data structures
    ...
    def setup_csv_logger(self):
        """Setup CSV logger for traffic data"""
        if self.config["log_data"]:
            self.csv_file = open("traffic_analysis.csv", mode="w", newline="")
            self.csv_writer = csv.writer(self.csv_file)
            self.csv_writer.writerow([
                "Timestamp", "Frame", "Current_Vehicles", "Total_Vehicles", 
                "Cars", "Bikes", "Buses", "Trucks", 
                "Congestion_Level", "Avg_Speed", "Suggested_Signal_Timing"
            ])
        else:
            self.csv_file = None
    ...
    def optimize_signal_timing(self):
        ...
        if congestion_level == 2:  # High congestion
            additional_time = min(60, base_time + vehicle_count * 2)
        elif congestion_level == 1:  # Medium congestion
            additional_time = min(45, base_time + vehicle_count)
        else:  # Low congestion
            additional_time = max(20, base_time - 10)
    ...
    def log_data(self, frame_id):
        """Log traffic data to CSV"""
        if self.csv_writer:
            self.csv_writer.writerow([
                datetime.now().isoformat(),
                frame_id,
                self.traffic_data["current_vehicles"],
                self.traffic_data["total_vehicles"],
                self.traffic_data["vehicle_types"]["car"],
                self.traffic_data["vehicle_types"]["motorbike"],
                self.traffic_data["vehicle_types"]["bus"],
                self.traffic_data["vehicle_types"]["truck"],
                self.traffic_data["congestion_level"],
                self.traffic_data["average_speed"],
                self.signal_timings[self.current_green]
            ])
    ...
    def start_processing(self):
        ...
        cv2.resizeWindow("Traffic Management System", 1200, 800)  # Set initial size
        ...
        while self.processing and self.cap.isOpened():
            ...
            if frame_id % self.config["detection_interval"] != 0:
                continue
            ...
            # Update signal control
            self.update_signal_control()
            ...
            if key == ord("q"):
                break
            elif key == ord("f"):  # Add fullscreen toggle with 'f' key
                cv2.setWindowProperty("Traffic Management System", cv2.WND_PROP_FULLSCREEN, 
                                     cv2.WINDOW_FULLSCREEN if cv2.getWindowProperty("Traffic Management System", cv2.WND_PROP_FULLSCREEN) == 0 else cv2.WINDOW_NORMAL)
    ...
    def get_current_stats(self):
        """Get current traffic statistics for dashboard"""
        if not self.dashboard_data:
            return {}
        return self.dashboard_data[-1]
    
    def get_historical_stats(self, limit=100):
        """Get historical traffic statistics for dashboard"""
        data_list = list(self.dashboard_data)
    ...
    def connect_to_simulator(self, simulator):
        """Properly connect to the urban simulator with bidirectional data exchange"""
        self.simulator = simulator
